...
def __biasquantityonposition_pair(input_data: BQOPInput) -> typing.Tuple[Decimal, Decimal]:
    element: BiasQuantityOnPositionElement = BiasQuantityOnPositionElement(input_data.maximum, input_data.target)
    context = fake_context()
    model_state = fake_model_state(price=fake_price(price=Decimal(100)),
                                   inventory=fake_inventory(base=input_data.current))
    buy, sell = element.process_order_pair(context, model_state, 0, input_data.buy, input_data.sell)
    if buy is None:
...
def test_from_args() -> None:
    args: argparse.Namespace = argparse.Namespace(biasquantityonposition_maximum_position=Decimal(
        17), biasquantityonposition_target_position=Decimal(7))
    actual: BiasQuantityOnPositionElement = BiasQuantityOnPositionElement.from_command_line_parameters(args)
    assert actual.maximum_position == 17
    assert actual.target_position == 7


def test_constructor() -> None:
    actual: BiasQuantityOnPositionElement = BiasQuantityOnPositionElement(Decimal(17), Decimal(7))
    assert actual.maximum_position == 17
    assert actual.target_position == 7
...
def test_table_target_zero() -> None:
    ...
    positions: typing.Sequence[int] = [0, 10, 20, 30, 40, 50, 60, -10, -20, -30, -40, -50, -60]
    adjusted_buys: typing.Sequence[int] = [10, 8, 6, 4, 2, 0, 0, 12, 14, 16, 18, 20, 20]
    adjusted_sells: typing.Sequence[int] = [10, 12, 14, 16, 18, 20, 20, 8, 6, 4, 2, 0, 0]
...
def test_table_negative_target() -> None:
    ...
    adjusted_sells: typing.Sequence[int] = [12, 14, 16, 18, 20, 20, 20, 10, 8, 6, 4, 2, 0]
    for index, position in enumerate(positions):
        buy_quantity, sell_quantity = __biasquantityonposition_pair(
            BQOPInput(buy, sell, Decimal(position), target, Decimal(50)))
        assert buy_quantity == Decimal(adjusted_buys[index])
...
def test_table_fractional_negative_target() -> None:
    ...
    adjusted_buys: typing.Sequence[Decimal] = [Decimal("0.125"), Decimal("0.0625"), Decimal(0), Decimal(
        0), Decimal(0), Decimal("0.1875"), Decimal("0.25"), Decimal("0.3125"), Decimal("0.375")]
    adjusted_sells: typing.Sequence[Decimal] = [Decimal("0.375"), Decimal("0.4375"), Decimal("0.5"), Decimal(
        "0.5"), Decimal("0.5"), Decimal("0.3125"), Decimal("0.25"), Decimal("0.1875"), Decimal("0.125")]
    for index, position in enumerate(positions):
        buy_quantity, sell_quantity = __biasquantityonposition_pair(
            BQOPInput(buy, sell, Decimal(position), target, Decimal(4)))
        assert buy_quantity == adjusted_buys[index]
...
def test_table_fractional_positive_target() -> None:
    ...
    target: Decimal = Decimal(2)
    positions: typing.Sequence[int] = [0, 1, 2, 3, 4, -1, -2, -3, -4]
    adjusted_buys: typing.Sequence[Decimal] = [Decimal("0.375"), Decimal("0.3125"), Decimal("0.25"), Decimal(
        "0.1875"), Decimal("0.125"), Decimal("0.4375"), Decimal("0.5"), Decimal("0.5"), Decimal("0.5")]
    adjusted_sells: typing.Sequence[Decimal] = [Decimal("0.125"), Decimal("0.1875"), Decimal("0.25"), Decimal(
        "0.3125"), Decimal("0.375"), Decimal("0.0625"), Decimal("0"), Decimal("0"), Decimal("0")]
    for index, position in enumerate(positions):
        buy_quantity, sell_quantity = __biasquantityonposition_pair(
            BQOPInput(buy, sell, Decimal(position), target, Decimal(4)))
        assert buy_quantity == adjusted_buys[index]
