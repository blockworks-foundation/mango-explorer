...
def context_has_default_values(ctx):
    ...
    assert ctx.group_name == "mango_test_v3.5"
    assert ctx.group_id == PublicKey("AnSZrDCTFcZuDCfEDLBjgyiLdp5gkDmVaREsriSBxQLH")
...
#     assert derived.group_name == "mango_test_v3.5"
#     assert derived.group_id == PublicKey("AnSZrDCTFcZuDCfEDLBjgyiLdp5gkDmVaREsriSBxQLH")
#     context_has_default_values(mango.Context.default())
...
def test_new_from_group_name():
    context_has_default_values(mango.Context.default())
    derived = mango.Context.default().new_from_group_name("mango_test_v3.5")
    assert derived.cluster == "devnet"
...
def test_new_from_group_id():
    context_has_default_values(mango.Context.default())
    derived = mango.Context.default().new_from_group_id(PublicKey("AnSZrDCTFcZuDCfEDLBjgyiLdp5gkDmVaREsriSBxQLH"))
    assert derived.cluster == "devnet"
    assert derived.cluster_url == "https://api.devnet.solana.com"
    assert derived.program_id == PublicKey("32WeJ46tuY6QEkgydqzHYU5j85UT9m1cPJwFxPjuSVCt")
    assert derived.dex_program_id == PublicKey("DESVgJVGajEgKGXhb6XmqDHGz3VjdgP7rEVESBgxmroY")
